...
public class TimerifficBackupAgent extends BackupAgentHelper {
    ...
    private static final String KEY_PREFS_STORAGE = "prefs_storage_";
    ...
    public void onCreate() {
        ...
        // Use the backup helper for the new app's pref storage.
        PrefsStorage ps = TimerifficApp.getInstance(this).getPrefsStorage();
        if (ps != null) {
            String name = ps.getFilename();
            FileBackupHelper helper = new FileBackupHelper(this, name);
            addHelper(KEY_PREFS_STORAGE + name, helper);
            Log.d(TAG, "Register backup 2 for " + name);
        }

        // --- profiles db backup ---

        // Use the backup helper for the profile database.
        ...
        if (filesPath != null && dbPath != null) {
            ...
            Log.d(TAG, "Register backup 3 for " + dbPath);
        }
